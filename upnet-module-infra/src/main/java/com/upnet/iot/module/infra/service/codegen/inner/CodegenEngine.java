package com.upnet.iot.module.infra.service.codegen.inner;
...
import com.upnet.iot.framework.apilog.core.annotation.ApiAccessLog;
import com.upnet.iot.framework.apilog.core.enums.OperateTypeEnum;
...
import com.upnet.iot.framework.common.pojo.PageParam;
...
import com.upnet.iot.framework.common.util.date.DateUtils;
import com.upnet.iot.framework.common.util.date.LocalDateTimeUtils;
import com.upnet.iot.framework.common.util.object.BeanUtils;
import com.upnet.iot.framework.common.util.object.ObjectUtils;
import com.upnet.iot.framework.common.util.string.StrUtils;
import com.upnet.iot.framework.excel.core.annotations.DictFormat;
import com.upnet.iot.framework.excel.core.convert.DictConvert;
import com.upnet.iot.framework.excel.core.util.ExcelUtils;
import com.upnet.iot.framework.mybatis.core.dataobject.BaseDO;
...
import com.upnet.iot.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.upnet.iot.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.upnet.iot.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.upnet.iot.module.infra.enums.codegen.CodegenSceneEnum;
import com.upnet.iot.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.upnet.iot.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.upnet.iot.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 upnet-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 upnet-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.upnet.iot.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/upnet-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/upnet-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "upnet-module-${table.moduleName}/" + // 顶级模块
                "upnet-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "upnet-module-${table.moduleName}/" + // 顶级模块
                "upnet-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "upnet-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "upnet-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "upnet-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "upnet-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
