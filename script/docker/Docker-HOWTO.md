...
依赖：docker compose v2，删除`name: upnet-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name upnet-maven-repo

docker run -it --rm --name upnet-maven \
    -v upnet-maven-repo:/root/.m2 \
